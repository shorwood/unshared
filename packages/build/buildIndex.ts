...
import { patternMatch } from '@unshared/string/patternMatch'
 ...
 */
export async function buildIndex(path: string = cwd(), options: BuildIndexOptions = {}): Promise<IndexFile[]> {
  const { pattern = '*.{ts,tsx,js,jsx}' } = options

  // --- Filter-in the import files and directories.
  ...
  for (const entity of directoryEntities) {
    try {
      // --- Find subdirectories with an index file.
      ...
      const isPatternMath = patternMatch(entity.name, pattern)
