...
type UseFileInputValue = Array<File | string> | File | string | undefined
...
function getThumbnail(item: File | string): string {
  return (item instanceof File)
    ? URL.createObjectURL(item)
    : item
}

export function useInputFile(initialValue: MaybeReference<UseFileInputValue>, options = {} as MaybeReference<UseFileInputOptions>) {
  // --- Initialize file array reference.
  const files = isRef(initialValue) ? initialValue : ref(initialValue)

  // --- Set or concat.
  const add = async(fileList: FileList, _options = options ) => {
    ...
    for (let index = 0; index < fileList.length; index++)
      fileArray.push(fileList.item(index)!)
