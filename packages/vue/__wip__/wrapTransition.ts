import { Transition, TransitionProps, VNode, defineComponent, h } from 'vue'
...
export function wrapTransition<T extends RawChildrenOrSlots>(vnode: T, options?: WrapTransitionOptions): T | VNode {
  ...
  // return h(Transition, transitionOptions, { default: () => vnode })
  // @ts-expect-error: ignore
  return Transition(transitionOptions, { slots: () => vnode })
}
...
if (import.meta.vitest) {
  ...
  const div = h('div', {}, { default: () => 'Hello' })

  it.only('should wrap a single VNode in a Transition component', () => {
    const result = wrapTransition(div, { enterActiveClass: 'fade-enter-active' })
    const component = h(defineComponent({ render: () => result }))
    ...
    // expectTypeOf(result).toEqualTypeOf<BaseTransition>()
  })
  ...
  //   expect(result.type).toBeInstanceOf(TransitionGroup)
