...
export async function useLeaflet(element: HTMLElement | string, options: MaybeRef<UseLeafletOptions>, markers: MaybeRef<Marker[]> = []) {
  ...
  const updateMap = (options: UseLeafletOptions) => {
    ...
    map.value.on('move', () => {
      const center = (map.value as Map).getCenter()
      lat.value = center.lat
      lng.value = center.lng
      zoom.value = (map.value as Map).getZoom()
    })
