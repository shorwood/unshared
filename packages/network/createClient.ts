...
export interface ClientOptions extends RequestInit {
  ...
  /**
   * The basic authentication credentials to use. If this is a string, it will be
   * used as the value of the `Authorization` header. If it is an object, the
   * `username` and `password` properties will be used to create a base64 encoded
   * string that will be used as the value of the `Authorization` header.
   *
   * @example `Zm9vOmJhcg==`
   */
...
export interface Client {
  ...
  $fetch<T>(path: string, options?: ClientOptions): Promise<Response<T>>
 ...
 * const client = createClient('https://example.com', { headers })
 * const response = await client.get('/api') // Promise<Response>
 */
export function createClient(url: string, options: ClientOptions = {}): Client {
