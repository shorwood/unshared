...
export interface ESBuildGlobSpecifierOptions {
  /**
...
export interface ESBuildGlobSpecifierPluginData {
  kind: ImportKind
  resolveDir: string
  resolvePaths: string[]
  sourcePaths: string[]
 ...
 */
export function ESBuildGlobSpecifier(options: ESBuildGlobSpecifierOptions = {}): Plugin {
  const {
  ...
  return {
    ...
    setup({ onResolve, onLoad }) {
      ...
      onLoad({ filter: PLUGIN_FILTER, namespace: PLUGIN_NAME }, ({ pluginData }) => {
        const { resolvePaths, resolveDir } = pluginData as ESBuildGlobSpecifierPluginData
...
export default ESBuildGlobSpecifier
