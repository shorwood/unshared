...
export type ValidationRule =
  ...
  const assertIsNumber: RuleFunction<number> = (value: unknown) =>
   typeof value !== 'number' ? throw new ValidationError('Value is not a number') : undefined
/**
 * Convert a validation rule to a `ValidatorFunction`.
 *
 * @param rule The rule to convert.
 * @returns The validation rule object.
 * @throws If the rule is not valid.
 * @example createRule(Number.isNaN) // { handler: Number.isNaN, parameters: [] }
 */
export function createRule(this: unknown, rule: ValidationRule): RuleFunction {
  // --- Return function as-is.
  if (typeof rule === 'function') return rule

  // --- Return a function that transforms the value to a Boolean.
  if (typeof rule === 'boolean') { return () => rule }

  // --- Return a function that validates the value against a RegExp.
  ...
  // --- Resolve rule object from array.
    ...
    if (typeof rule[0] === 'function') {
      ...
      return handler.bind(this, firstParameter, ...parameters)
    }
