...
/**
 * This class is used to create errors that provide a consistent interface for
 * error handling and should be used for either **all** or **none** of the errors
 * in a project.
 *
...
export class Exception extends Error {
  constructor(message: string, options?: Partial<Exception>)
  constructor(...parameters: Parameters<typeof Error>) {
    super(...parameters)
    Error.captureStackTrace(this, Exception)

    // --- Decompose the parameters.
    const [, options] = parameters as [string, Exception]
  ...
  /**
   * Wrap an `Error` in an `Exception`.
   *
   * @param error The error to wrap.
   * @param options The additionnal properties to set on the exception.
   * @returns The wrapped exception.
   */
  public static wrap(error: Error, options?: Partial<Exception>): Exception {
...
if (import.meta.vitest) {
  it('should create an instance of basic error', () => {
    const error = new Exception('An error occurred while processing your request.')
